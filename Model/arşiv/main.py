...
class ZemberekClient:
    ...
    def analyze_sentence(self, text: str):
        ...
        try:
            ...
            for r in resp.results:
                ...
                if best is not None:
                    lemmas = list(getattr(best, "lemmas", [])) or list(getattr(best, "lemma", []))
                    pos = getattr(best, "pos", "") or getattr(best, "posShort", "")
                else:
                    lemmas, pos = [], ""
    ...
    def spell_check(self, word: str) -> tuple[bool, str]:
        ...
        try:
            ...
            return resp.is_correct, getattr(resp, "error", "")
        except Exception as e:
            return False, str(e)
...
if __name__ == "__main__":
    zc = ZemberekClient(host="localhost", port=6789)
    # Dokunma: normalize etme/düzeltme (kurum kısaltmaları vb.)
    do_not_touch = {"OG", "trafo", "klemens"}

    # KAVRAM SÖZLÜĞÜ (örnek): domain kelimeleriniz – mümkünse kök hâlleri
    concept_dict = {
        "sigorta", "faz", "problem", "ariza", "arıza", "topraklama",
        "klemens", "kablo", "kontakt", "kaçak", "hat", "trafo", "direk", "ayırıcı"
    }

    sample = "OG trafo kapplo klemens sahasında sigora var gibi; ayrica arizaa fazproblemi tespit edildi."
    
    # sample = "arizanin"
    resp = zc.morph.AnalyzeSentence(morph_pb.SentenceAnalysisRequest(input=sample))
    resp
    out = preprocess_text_with_concepts(
        sample, zc,
        concept_dict=concept_dict,
        do_not_correct=do_not_touch,
        max_edit_dist=2,
    ...
    print("CONC:", out["concept_changes"])
    print("COMP:", out["compound_changes"])
    print("FINAL TEXT:", out["final_text"])
    print("LEMMAS:", out["lemmas"])
